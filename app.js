...
function renderFoodResults(query = "") {
  const results = query ? foodCatalog.filter((food) => food.name.includes(query)) : [];
  $("#food-results").innerHTML = results.length ? results.map((food) => `<article class="food-result"><span class="food-result-icon">${food.emoji}</span><div><strong>${food.name}</strong><small>${food.kcal} kcal · P ${food.protein}g · F ${food.fat}g · C ${food.carbs}g / ${food.unit}</small></div><button data-add-food="${food.name}">＋追加</button></article>`).join("") : (query ? `<p class="food-result-empty">「${query}」に一致する食品が見つかりません。別の料理名や商品名で検索してください。</p>` : "");
  $$('[data-add-food]').forEach((button) => button.addEventListener("click", () => addCatalogFood(button.dataset.addFood)));
}
...
function renderAnalysisResult(result, name, source = "入力内容から推定") {
  ...
  box.hidden = false;
  box.innerHTML = `<h3>解析結果 <span style="color:var(--green);font-size:10px;font-weight:500">${name}</span></h3><p style="color:var(--muted);font-size:9px;margin:0 0 10px">${source} · 記録前に内容を確認してください</p><div class="result-grid"><div class="result-stat"><span>カロリー</span><strong>${formatNumber(result.kcal)}</strong><small>kcal</small></div><div class="result-stat"><span>たんぱく質</span><strong>${result.protein}g</strong></div><div class="result-stat"><span>脂質</span><strong>${result.fat}g</strong></div><div class="result-stat"><span>炭水化物</span><strong>${result.carbs}g</strong></div></div><div class="result-actions"><button class="edit-result" id="edit-result">内容を調整</button><button class="save-result" id="save-result">この内容で記録</button></div>`;
  $("#save-result").addEventListener("click", () => { meals.push({ id: Date.now(), type: "夕食", time: new Date().toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit" }), name, detail: "AI解析 · 内容を確認済み", emoji: "🍽️", ...result }); renderMeals(); renderSummary(); closeModal(); showToast("食事を記録しました"); });
  $("#edit-result").addEventListener("click", () => $("#meal-description").focus());
}

function analyze() { const hasImage = $("#meal-image").files && $("#meal-image").files.length > 0; const description = $("#meal-description").value.trim() || "鶏むね肉、玄米、ブロッコリー"; const result = estimateMeal(description); const name = hasImage ? "アップロード画像の食事" : description.split(/[、,]/)[0] + "の食事"; const source = hasImage ? "写真から推定（デモ）" : "入力内容から推定"; analysis = result; $("#analyze-button").innerHTML = "<span>✓</span> 解析が完了しました"; setTimeout(() => { $("#analyze-button").innerHTML = "<span>✦</span> AIで栄養を解析する"; }, 1600); renderAnalysisResult(result, name, source); }
